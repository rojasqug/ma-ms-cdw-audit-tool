...
// Softer, lighter status colors (still distinct)
const STATUS_COLORS = {
  'NEW TICKET': '#2684FF',                 // Blue 400 (lighter than 600)
  'INITIAL REVIEW': '#FFAB00',             // Yellow 400 (slightly darker for readability)
  'EMAIL TO VENDOR': '#FF7452',            // Red 300
  'DATA REMEDIATION IN PROGRESS': '#00B8D9', // Teal 400 (darker for readability)
  'DATA REMEDIATION COMPLETED': '#36B37E',   // Green 400 (darker for readability)
  'NEED MORE INFO': '#8777D9',             // Purple 300
  'DECLINED': '#FFB57D',                   // Orange 300
  'DONE': '#4CC3A6',                       // Soft green (distinct from completed)
};
...
function getStatusTextColor(status, baseHex) {
  ...
  // Specific request: keep text yellow (but darker) for Initial Review
  if (norm === 'INITIAL REVIEW') {
    // Slightly darker mustard yellow for text
    return '#CC8B00';
  }
...
// Choose an accessible text color: for very bright base colors (e.g., yellow),
// use a darker shade of the same color (not black) for better readability.
...
// Resolve a color for a given status name, using explicit map first,
// then a stable hash into CHART_COLORS to avoid duplicates.
...
// Deterministic agent color so a given agent keeps the same bar color
...
// Pick a fill color for a bar ensuring it's not already used by the map
...
// Custom legend: keep colored dot, force black text
...
function sanitizeInvokeError(err) {
  // Prefer showing the actual error from resolver/bridge
...
// StatusTag replaced by custom StatusBadge (non-removable and color-mapped)
...
function HistoryModal({ issueKey, onClose, colorMap }) {
  ...
  const fetchHistory = React.useCallback(async () => {
    try {
      ...
      console.error('[CDRTable] getIssueStatusHistory failed', primaryErr);
      setError(primaryErr);
    } finally {
      setLoading(false);
    }
...
export default function CDRTable() {
  ...
  const fetchIssues = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const resp = await bridge.invoke('searchCDROpenRemediation');
      if (resp?.error) throw new Error(resp.error);
      setIssues(resp?.issues || []);
    } catch (primaryErr) {
      // eslint-disable-next-line no-console
      console.error('[CDRTable] searchCDROpenRemediation failed', primaryErr);
      setError(primaryErr);
    } finally {
      setLoading(false);
    }
  ...
  // Build filter options from loaded issues
  ...
  // Apply filter to issues for charts and table
  ...
  // Build a status->color map for statuses present to guarantee distinct colors
  const statusColorMap = useMemo(() => {
    const present = new Set((filteredIssues || []).map(it => normalizeStatus(it.status || 'Unknown')));
    const map = new Map();
    // first, set colors for any present statuses that exist in STATUS_COLORS
    present.forEach(name => {
      if (STATUS_COLORS[name]) map.set(name, STATUS_COLORS[name]);
    });
    // then assign remaining distinct colors from fallback palette
    let i = 0;
    ...
    (filteredIssues || []).map((it) => ({
      ...
      cells: [
        ...
        { key: 'currentStatus', content: formatDuration(it.currentStatusDurationMs || 0) },
        {
          key: 'history',
          content: (
            <Button appearance="primary" onClick={() => openHistory(it.key)}>
              View Status History
            </Button>
          ),
        },
  ...
  const assigneeData = useMemo(() => {
    ...
    // Sort by count desc, cap to topN and bucket the rest under 'Others'
    const topN = 10;
  ...
  // Deterministic, non-repeating colors for agents across the full dataset
  const agentColorMap = useMemo(() => {
    const names = new Set();
    for (const it of issues || []) {
      names.add(it.assignee?.name || 'Unassigned');
    }
    const list = Array.from(names).sort((a, b) => a.localeCompare(b)); // stable ordering
    const map = new Map();
    list.forEach((n, i) => {
      if (i < CHART_COLORS.length) {
        map.set(n, CHART_COLORS[i]);
      } else {
        // HSL fallback to avoid duplicates when exceeding palette size
  ...
  return (
    <Box xcss={wrapStyles}>
      {/* Top toolbar with description and filter */}
                ...
                <PieChart>
                  <Pie
                    ...
                    label={({ cx, cy, midAngle, outerRadius, percent }) => {
                      ...
                      const radius = outerRadius + 18; // place label outside with extra padding
            ...
            <Box xcss={chartInner}>
              <ResponsiveContainer width="100%" height={460}>
                <BarChart data={assigneeData} margin={{ top: 8, right: 24, left: 24, bottom: 52 }} barCategoryGap="6%" barGap={2}>
                  ...
                  {/* Single-series legend removed to reduce clutter */}
      ...
      <Box xcss={tableWrap}>
        ...
        {!loading && !error && (filteredIssues?.length || 0) === 0 && (
          <EmptyState
            header="No open Remediation Request issues in CDR"
